...
tmp4 = r1.*qfun2(s2-s3,s2,zetaN.D2)+ (1-r1).*r2.*qfun2(s1-s3,s1,zetaN.D2) + (1-r1).*(1-r2).*qfun2(s1-s2,s1,zetaN.D2);
tmp5 = (1-r1).*(1-r2).*qfun2(s2-s3,s2,zetaN.D3);
Q = sum(tmp1 + tmp2 + tmp3 + tmp4 + tmp5);
    ...
    function q= qfun2(x,xx,theta)
        %sigma = theta.a*xx+ theta.b;
        sigma=sigmaVec(theta,xx);
        q = 2*log(sigma) + (x./sigma).^2;
        q = -0.5*q;
    end
end
