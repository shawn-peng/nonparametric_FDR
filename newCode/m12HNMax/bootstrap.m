...
for dd=1:length(ds)
   D=load(['test_search/matdata/',ds{dd}]);
    ...
    if ~all(mat(:,1)==mat(:,2)) && length(mat)>100
        for i=1:B
            ...
            if i==1
                ...
                for jj = 1:size(signs,1)
                    [SS.zeta{jj},SS.ll(jj),SS.lls{jj},SS.k(jj)] = EM12HNMax(matb,signs(jj,1),signs(jj,2),K);
                ...
                S.lls{i}=SS.lls{JJ};
                S.k(i)=SS.k(JJ);
                S.LambdaSign=signs(JJ,:);
            else
                S.LambdaSign=signs(JJ,:);
                [S.zeta{i},S.ll(i),S.lls{i},S.k(i)] = EM12HNMax(matb,signs(JJ,1),signs(JJ,2),K);
            end
            %[S.zeta{i},S.ll(i),S.lls{i},S.k(i)] = EM12HNMax(matb,-1,1,K);
        ...
        %[S.pdfM_2,S.pdfC_2,S.pdfI1_1, S.pdfI1_2]=s2Dens(S.s2,S.zeta);
         ...
         plotPdfs(S.s1{i},S.pdfM{i},S.pdfC{i},S.pdfI1{i})
         subplot(1,2,2);
