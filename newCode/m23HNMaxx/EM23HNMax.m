...
%     end
    ...
    I1C=[s1;s2;s1];
    tmp_ss1=opts.NNSigma.predict(s1);
    tmp_mu1=opts.NNMu.predict(s1);
    tmp_ss2=opts.NNSigma.predict(s2);
    tmp_mu2=opts.NNMu.predict(s2);
    s_I2NT=nonTruncNSample(tmp_mu1,tmp_ss1,s_I2);
    s_I2R= tmp_mu2 + sqrt(tmp_ss2).*randn(M,1);
    s_I2T= truncBetweenNSample(tmp_mu1, tmp_ss1, s_I2);
    s_I2C = [s_I2NT;s_I2R;s_I2T];
    tmp_muC=[tmp_mu1;tmp_mu2;tmp_mu1];
    tmp_ssC=[tmp_ss1;tmp_ss2;tmp_ss1];
    %s_I2C = [s_I2NT];
    zetaN9 = zetaN;
    zetaN9.D2.netMu=fitNNMu(I1C,s_I2C,r_I2, tmp_ssC, opts);
    zetaN9.D2.netSS=fitNNSigma(I1C, s_I2C, r_I2, tmp_muC, opts);
